...
%     ];
windows = [datetime(1959, 10, 1), datetime(2019, 12,31);   % Largest sample (1959-2019)
    datetime(1959, 10, 1), datetime(1984, 10, 1);          % Pre 84
    datetime(1985, 1 , 1), datetime(2019, 12 ,31);          % Post 84 (1985-2019)
    datetime(1973, 1,  1), datetime(2019, 12, 31)           % Post Bretton Woods (1973-2019)
    ];
...
for jSpec = 1:nSpec
    for jInit = 1:length(initSettingVec)
        % Load results file
        pathFigs = [pathFiguresBase 'initSetting=' initSettingVec{jInit} '_'];
...
f.Units = 'inches';
f.Position(3:4) = [8,8];
saveas(gcf, [pathFiguresBase 'loglik.png'])
...
f.Units = 'inches';
f.Position(3:4) = [8,8];
saveas(gcf, [pathFiguresBase 'aad.png'])
...
writecell(out, [pathFiguresBase 'loglik.xls'])

out = num2cell(aad_mat);
out = [{Spec.nameDate}' out];
out = [{''} initSettingVec; out];
writecell(out, [pathFiguresBase 'aad.xls'])
