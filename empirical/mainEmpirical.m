...
findGlobal = 1;  % Use GlobalSearch for PML
...
for j = 1:nSpec
    ...
    % Store PML and cumulant results
    [teststat, teststats_boot, A, c, shocks, ...
        H_estim(:,:,1),         H_estim(:,:,2),...
        C_estim_raw(:,:,1), C_estim_raw(:,:,2)] = ...
        var_test_indep_emp(Y, Z, p, pml_settings, [], verbose, findGlobal);
    
           
    
    % Flip signs and permute estimated H to find best fit to reference
    % matrices
...
colormap(cm)
sgtitle('C, ugap')
saveas(gcf, [pathFigs 'C_ugap_findGlobal=' num2str(findGlobal) '.png'])
...
colormap(cm)
sgtitle('C, ygap')
saveas(gcf, [pathFigs 'C_ygap_findGlobal=' num2str(findGlobal) '.png'])
