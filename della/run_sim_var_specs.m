% run_sum_var_specs.m  Same as run_sim_var, except adapted to serially run
...
tauVec  = [0, 0.4];
numsim  = 5000;  % Could be vector input
numboot = 2000;        % No. of bootstrap iterations
...
% Store combinations to be run.
Combs         = array2table(tauVec(:), 'VariableNames', {'tau'});
nSpec         = height(Combs);
...
for jSpec = 1:nSpec 
    ...
    save([outPath 'out_numboot=' num2str(numboot) ...
          ',numsim=' num2str(numsim), ...
          ',tau=' num2str(tau),...
          ',T=' num2str(T) '.mat'], '-v7.3')
end
